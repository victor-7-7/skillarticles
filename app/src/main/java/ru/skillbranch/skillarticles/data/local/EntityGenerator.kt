...
object EntityGenerator {
    fun generateArticles(count: Int): List<ArticleRes> =
        Array(count) { articleItems[it % 6] }
            ...
            .mapIndexed { index, article ->
                article.copy(
                    data = article.data.copy(
                        id = "$index", date = Date().add(-index, TimeUnits.DAY)
                    ),
                    counts = article.counts.copy(
                        comments = (20..100).random(),
    ...
    fun generateComments(count: Int): List<CommentRes> = Array(count) {
        CommentRes(
            ...
            date = Date().add(-count.inc(), TimeUnits.DAY).add(it, TimeUnits.DAY),
            slug = "$it/"
        )
    }
        .toList()
    ...
    private fun createArticleItem(articleId: String): ArticleRes {
        val article = articleItems[articleId.toInt() % 6]
        return article.copy(
            data = article.data.copy(id = articleId),
            counts = article.counts.copy(
                comments = (10..40).random(), readDuration = (2..10).random(), likes = (15..100).random()
            )
        )
    }
        ...
        1 -> ArticleRes(
            data = ArticleDataRes(
                ...
                category = CategoryRes(
                    "0",
                    icon = "https://skill-branch.ru/img/mail/bot/android-category.png",
                    title = "Android"
                ),
