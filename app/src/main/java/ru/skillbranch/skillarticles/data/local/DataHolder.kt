...
object LocalDataHolder {
    ...
    private val articleInfo = MutableLiveData<ArticlePersonalInfo3?>(null)
    ...
    fun findArticlePersonalInfo(articleId: String): LiveData<ArticlePersonalInfo3?> {
        GlobalScope.launch {
            delay(500)
            articleInfo.postValue(ArticlePersonalInfo3(isBookmark = true))
        }
        return articleInfo
    }

    fun updateArticlePersonalInfo(info: ArticlePersonalInfo3) {
...
object NetworkDataHolder {
    ...
    private val comments: Map<String, MutableList<CommentRes>> = articles.associate { article ->
        article.data.id to EntityGenerator.generateComments(article.counts.comments) as MutableList
    }

    fun commentsCount(articleId: String) = comments[articleId]?.size ?: 0
    ...
    fun sendMessage(articleId: String, message: String, answerId: String?) {
        ...
        var comment =
            CommentRes(id = nextId, user = me, body = message, date = Date(), slug = "$nextId/")
        if (answerId == null) {
...
data class ArticlePersonalInfo3(
