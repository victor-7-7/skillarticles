...
class ContainerBehavior() : AppBarLayout.ScrollingViewBehavior() {
    ...
    override fun onMeasureChild(
        ...
        if (child is FragmentContainerView
            && !child.children.first().isNestedScrollingEnabled
        ) {
            val appbar = parent.children.find { it is AppBarLayout }
            val abh = appbar?.measuredHeight ?: 0
            val bottombar = parent.children.find { it is BottomNavigationView }
            val bbh = if (bottombar?.isVisible == true)
                bottombar.measuredHeight else 0
            val height = View.MeasureSpec.getSize(parentHeightMeasureSpec) - abh - bbh
