...
class MarkdownCodeView private constructor(
    ...
    init {
        tv_codeView = MarkdownTextView(context, fontSize * .85f).apply {
        ...
        sv_scroll = object : HorizontalScrollView(context) {
            ...
            // add code text to sv_scroll
            addView(tv_codeView)
        }
        // add sv_scroll to MarkdownCodeView
        addView(sv_scroll)
    ...
    override fun renderSearchPosition(position: Pair<Int, Int>, offset: Int) {
        super.renderSearchPosition(position, offset)
        if ((parent as ViewGroup).hasFocus() && !tv_codeView.hasFocus())
            tv_codeView.requestFocus()
        Selection.setSelection(spannableContent, position.first.minus(offset))
